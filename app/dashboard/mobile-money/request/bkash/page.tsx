...
export default function DashboardBkashPage() {
  ...
  const onSubmit = async (data: MobileMoneyFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('bKash request created successfully!')
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 p-4 sm:p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">bKash Request</h1>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Send money via bKash</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
              ...
              <div className="bg-pink-50 dark:bg-pink-900/20 border border-pink-200 dark:border-pink-800 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-medium text-pink-900 dark:text-pink-100">bKash</h3>
                    <p className="text-sm text-pink-700 dark:text-pink-200">Mobile financial service</p>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-pink-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {isSubmitting ? (
                  <>
                    <LoadingSpinner size="sm" className="mr-2" />
                    Creating bKash Request...
                  </>
                ) : (
                  <>
                    <DevicePhoneMobileIcon className="h-5 w-5 mr-2" />
                    Create bKash Request
                  </>
                )}
              </button>
            </form>
